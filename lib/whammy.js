...
window.Whammy = (function(){
	...
	function toWebM(frames, outputAsArray){
		...
		var CLUSTER_MAX_DURATION = 30000|0;
	...
	function checkFrames(frames){
		...
		for(var i = 1; i < frames.length; i++){
			var frame=frames[i];
			if(frame.width != width) throw "Frame " + (i + 1) + " has a different width";
			if(frame.height != height) throw "Frame " + (i + 1) + " has a different height";
			if(frame.duration < 0 || frame.duration > 0x7fff) throw "Frame " + (i + 1) + " has a weird duration (must be between 0 and 32767)";
